...
wait1 = 0
resetmove1 = 0
function januszonStart( agent, actorKnowledge, time)
        agent:selectWeapon(Enumerations.Chaingun);
end

function januszwhatTo( agent, actorKnowledge, time)
        mindist = 9999
		mindisttrig = -1
        ...
        if ( actorKnowledge:isMoving()==false or resetmove1 == 1) then
				resetmove1 = 0
						...
						if (isok == 1) then
							...
							if(hasNoAmmo == 0 and dystToEn > 100 and actorKnowledge:getAmmo(actorKnowledge:getWeaponType()) > 0) then
								if(enemis:at(en):getWeaponType() == Enumerations.Railgun or enemis:at(en):getWeaponType() == Enumerations.Chaingun) then
									vec = Vector4d( -vectorToEn:value(1), vectorToEn:value(0), 0, 0):normalize() * 15
